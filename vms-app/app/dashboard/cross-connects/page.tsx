      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            End-to-End pathway mapping with deep port validation.
          </p>
        </div>
        <div className="flex gap-3">
          <button
            onClick={fetchCrossConnects}
            className="px-5 py-2.5 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-sm font-semibold border border-slate-700 transition-all"
          >
            Refresh List
          </button>
          <button
            onClick={() => setIsModalOpen(true)}
            className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-sm font-semibold shadow-[0_0_20px_-5px_rgba(59,130,246,0.5)] transition-all"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                {crossConnects.map((cx, i) => (
                  ...
                  >
                    ...
                    <div className="flex gap-2 ml-4">
                      {cx.status === "Requested" && (
                        <button
                          onClick={() => handleUpdateStatus(cx.id, "Active")}
                      ...
                      <button
                        onClick={() => handleEditClick(cx)}
                        className="text-indigo-400 hover:text-indigo-300 p-1.5 rounded-lg hover:bg-indigo-500/10 transition-colors"
                        title="Edit"
                      >
                        <Pencil className="w-4 h-4" />
                      </button>
        ...
        <div className="col-span-1 bg-card/40 border border-border/50 rounded-2xl p-6 backdrop-blur-xl">
          ...
          <p className="text-sm text-slate-400 leading-relaxed mb-6">
            The VMS infrastructure automatically blocks duplicate logical port
            mappings before they are provisioned to the NOC team, ensuring 100%
            data integrity inside your physical environment.
          </p>
          <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-800">
            <div className="flex justify-between items-center mb-2">
              <span className="text-xs text-slate-400">
                Collisions Prevented (30d)
              </span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="w-6 h-6 rounded bg-indigo-500/20 flex items-center justify-center text-xs font-bold text-indigo-300 border border-indigo-500/30">
                          A
                        </div>
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          placeholder="Enter Side A Company..."
                          value={formData.sideACompany}
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              sideACompany: e.target.value,
                            })
                          }
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              sideAEquipmentId: e.target.value,
                              sideAPortId: "",
                            })
                          }
                        ...
                        >
                          ...
                          {formData.sideARackId &&
                            racks
                              .find(
                                (r) => r.id === parseInt(formData.sideARackId),
                              )
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              sideAPortId: e.target.value,
                            })
                          }
                          className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-100 focus:ring-1 focus:ring-blue-500 outline-none disabled:opacity-50"
                        >
                          <option value="">Select Port...</option>
                          {formData.sideAEquipmentId &&
                            racks
                              .find(
                                (r) => r.id === parseInt(formData.sideARackId),
                              )
                              ?.equipments?.find(
                                (e: any) =>
                                  e.id === parseInt(formData.sideAEquipmentId),
                              )
                              ?.ports?.map((p: any) => (
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="w-6 h-6 rounded bg-emerald-500/20 flex items-center justify-center text-xs font-bold text-emerald-300 border border-emerald-500/30">
                          Z
                        </div>
                        <span className="text-sm font-medium text-slate-300">
                          Side Z Configuration
                        </span>
                      </div>
                      <div>
                        <label className="block text-xs text-slate-500 mb-1">
                        ...
                        <input
                          type="text"
                          placeholder="Enter Side Z Company..."
                          value={formData.sideZCompany}
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              sideZCompany: e.target.value,
                            })
                          }
                          className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-100 focus:ring-1 focus:ring-blue-500 outline-none"
                        />
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              sideZEquipmentId: e.target.value,
                              sideZPortId: "",
                            })
                          }
                          className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-100 focus:ring-1 focus:ring-blue-500 outline-none disabled:opacity-50"
                        >
                          <option value="">Select Equipment...</option>
                          {formData.sideZRackId &&
                            racks
                              .find(
                                (r) => r.id === parseInt(formData.sideZRackId),
                              )
                              ?.equipments?.map((eq: any) => (
                                <option key={eq.id} value={eq.id}>
                                  {eq.name} ({eq.equipmentType})
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              sideZPortId: e.target.value,
                            })
                          }
                          className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-100 focus:ring-1 focus:ring-blue-500 outline-none disabled:opacity-50"
                        >
                          <option value="">Select Port...</option>
                          {formData.sideZEquipmentId &&
                            racks
                              .find(
                                (r) => r.id === parseInt(formData.sideZRackId),
                              )
                              ?.equipments?.find(
                                (e: any) =>
                                  e.id === parseInt(formData.sideZEquipmentId),
                              )
                              ?.ports?.map((p: any) => (
                                <option key={p.id} value={p.id}>
                                  {p.portName}
