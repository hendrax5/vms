...
const StatusBadge = ({ status }: { status: string }) => {
    ...
    if (status === 'Approved') return <span className="inline-flex items-center gap-1.5 py-1 px-3 rounded-md text-xs font-medium bg-blue-500/10 text-blue-400 border border-blue-500/20"><CheckCircle className="w-3 h-3" /> Approved</span>;
    if (status === 'KioskVerified') return <span className="inline-flex items-center gap-1.5 py-1 px-3 rounded-md text-xs font-medium bg-indigo-500/20 text-indigo-400 border border-indigo-500/40 animate-pulse"><Camera className="w-3 h-3" /> Kiosk Verified</span>;
